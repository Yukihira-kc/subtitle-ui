...
let settings={columns:20,mode:'page',keyColor:'#00ff00',fontSize:32,lineCount:2,captionFont:'noto-sans-jp',textColor:'#ffffff',outline:true,fontWeight:'normal'},online={},active=null;
...
$('through-mode').onclick=()=>{if(!ready)return;socket.emit('setThroughMode',{enabled:!throughMode},result=>{if(result&&result.ok){throughMode=!!result.enabled;updateThroughMode();status(throughMode?'スルーモード中：新規入力を送信待機へ送ります':'通常モード：新規入力を編集欄へ追加します');}});};
...
socket.on('version',v=>{$('version').textContent=v==='v27'?'v27':'v27 / サーバー '+v;});
...
socket.on('reviewItem',item=>{if(!throughMode)receive(item);});socket.on('subtitleLog',addHistory);socket.on('captionQueue',renderQueue);socket.on('caption',display);socket.on('settings',applySettings);socket.on('throughMode',enabled=>{throughMode=!!enabled;updateThroughMode();});socket.on('typing',typing);socket.on('presence',s=>{online=s;monitor();});socket.on('active',s=>{active=s;monitor();});
